...
app.get("/health", (_req, res) => {
  res.json({
    ...
    port: PORT,
    policies_loaded: Object.keys(policies).length,
    roles_loaded: Object.keys(roleMappings).length,
  });
});
...
app.post("/api/tools/call", async (req, res) => {
  try {
    ...
    console.error(`[GovernanceMCP] Error: ${error.message}`);
...
// ─── Start Server ────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`[GovernanceMCP] Governance MCP Server running on port ${PORT}`);
  console.log(`[GovernanceMCP] Loaded ${Object.keys(policies).length} policies, ${Object.keys(roleMappings).length} role mappings`);
});
